# -*- coding: utf-8 -*-
"""
This script generates image chips based on a raster image.  It also creates
masks for segmentation, if provided with a labels vector file.
"""
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(
        description="Extract chips from a raster file, and optionally generate mask chips",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2,
        help="(for 'percentiles' mode) lower cut of percentiles for cumulative count in intensity rescaling",
    )
    parser.add_argument(
        "--upper-cut",
        type=float,
        default=98,
        help="(for 'percentiles' mode) upper cut of percentiles for cumulative count in intensity rescaling",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="specify band indexes. If type is 'jpg', defaults to (1, 2, 3). If type is 'tif', defaults to the total band count.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="satlomasproc {ver}".format(ver=__version__),
    )
    parser.add_argument(
        "-v",
        "--verbose",
