...
class TrainingScriptConfig(object):
    ...
    def __str__(self):
        return "esp:{}_eps:{}_loss:{}_opt:{}".format(
            self.early_stop_patience, self.epochs, self.model_loss, self.optimizer
        )
...
class LSTMTrainingScriptConfig(TrainingScriptConfig):
    ...
    def __str__(self):
        ...
        base_conf_str = (
            self.base_config["first_layer"]["mult"]
            + self.base_config["first_layer"]["dropout_rate"]
            + self.base_config["last_layer"]["mult"]
            + self.base_config["last_layer"]["dropout_rate"]
        )

        mid_conf_str = (
            self.mid_layers_config["n_layers"]
            + self.mid_layers_config["mult"]
            + self.mid_layers_config["dropout_rate"]
        )
...
class LSTMHyperoptTrainingScriptConfig(LSTMTrainingScriptConfig):
    ...
    def __str__(self):
        ...
        hyperopt_pars_str = (
            str(self.hyperopt_pars["mid_layers"])
            + str(self.hyperopt_pars["mults"])
            + str(self.hyperopt_pars["dropout_rate_range"])
            + str(self.hyperopt_pars["max_evals"])
        )

        return "{}_hyperoptpars:{}".format(super_str, hyperopt_pars_str)
