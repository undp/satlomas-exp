...
To run this script :
    $ satlomas_lstm_train_hyperopt config_train_lstm_temp.json -vv
...
def train_lstm(script_config):
    ...
    # Leer el dataset
    raw_dataset = read_time_series_from_csv(
        input_csv, date_col, hr_col, numeric_var, sensor_var
    )
    _logger.debug("Dataset of shape {} read".format(raw_dataset.shape))

    # Obtener la variable de interes del dataset
    time_series_dset = get_interest_variable(
        raw_dataset, sensor_var, date_col, hr_col, numeric_var, target_sensor
    )
    _logger.debug(
    ...
    dataset_splits = train_val_test_split(
        sup_dataset, n_past_steps, n_features, numeric_var
    )
    _logger.debug("Got split:")
    for key in dataset_splits.keys():
        _logger.debug(
            "{} shapes: {},{}".format(
                key, dataset_splits[key]["X"].shape, dataset_splits[key]["y"].shape
            )
        )
    ...
    _logger.debug(
        "LSTM NNNet hyperpars to optimize on: mults:{}, dropout:{}, n mid layers:{}".format(
        ...
        [
            {
                ...
                "dropout_rate_1": hp.uniform(
                    "dropout_rate_1", dropout_rate_range[0], dropout_rate_range[1]
                ),
                "mult_mid": hp.choice("mult_mid", mults),
                "dropout_rate_mid": hp.uniform(
                    "dropout_rate_mid", dropout_rate_range[0], dropout_rate_range[1]
                ),
                "mult_n": hp.choice("mult_n", mults),
                "dropout_rate_n": hp.uniform(
                    "dropout_rate_n", dropout_rate_range[0], dropout_rate_range[1]
                ),
    ...
    optimal_pars = fmin(
        get_lstm_nnet_opt, space, algo=tpe.suggest, max_evals=hyperopt_pars["max_evals"]
    )

    opt_time = time.time() - tic
    _logger.debug(
        "Hyper parameter optimization for optimal pars {} took {} seconds for {} datapoints".format(
    ...
    lstm_nnet_arq = build_lstm_nnet(
        trainset["X"], base_config_opt, mid_layers_config_opt, model_loss, optimizer
    )
    _logger.debug(
    ...
    tic = time.time()
    train_mae = eval_regression_performance(
        trainset, lstm_nnet, scaler, measure=mean_absolute_error
    )
    train_eval_time = time.time() - tic
    _logger.debug("Train MAE {} and took {} seconds".format(train_mae, train_eval_time))

    tic = time.time()
    test_mae = eval_regression_performance(
        dataset_splits["testset"], lstm_nnet, scaler, measure=mean_absolute_error
    )
    test_eval_time = time.time() - tic
    _logger.debug("Test MAE {} and took {} seconds".format(test_mae, test_eval_time))

    train_r2 = eval_regression_performance(
        trainset, lstm_nnet, scaler, measure=r2_score
    )
    _logger.debug("Train R2 {}".format(train_r2))

    test_r2 = eval_regression_performance(
        dataset_splits["testset"], lstm_nnet, scaler, measure=r2_score
    )
...
def parse_args(args):
    ...
    parser.add_argument(
        "--version", action="version", version="satlomas {ver}".format(ver=__version__)
    )
    parser.add_argument(
        dest="config_file",
...
def main(args):
    ...
    _logger.debug(
        "Starting training an LSTM with {} configuration".format(args.config_file)
    )
